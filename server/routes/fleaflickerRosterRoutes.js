...
router.get('/league/:leagueId/data', async (req, res, next) => {
    ...
    try {
        ...
        const [fleaflickerResponse, fantasyCalcResponse, enrichedPlayerMap] = await Promise.all([
            axios.get(`https://www.fleaflicker.com/api/FetchLeagueRosters?sport=NFL&league_id=${leagueId}&season=${season}`),
            axios.get(`https://api.fantasycalc.com/values/current?isDynasty=true&numQbs=1&ppr=0.5`),
            loadEnrichedPlayerData()
        ]);
        ...
        const allEnrichedRosters = fleaflickerRosters.map(roster => {
            ...
            const finalRosterPlayers = playersOnRoster.map(playerData => {
...
router.get('/league/:leagueId/roster/:rosterId', async (req, res, next) => {
    ...
    try {
        ...
        const [fleaflickerResponse, fantasyCalcResponse, enrichedPlayerMap] = await Promise.all([
            axios.get(`https://www.fleaflicker.com/api/FetchLeagueRosters?sport=NFL&league_id=${leagueId}&season=${season}`).then(r => r.data),
            axios.get(`https://api.fantasycalc.com/values/current?isDynasty=true&numQbs=1&ppr=0.5`).then(r => r.data),
            loadEnrichedPlayerData()
        ]);
        ...
        const ownerName = targetRoster.team?.name || 'Unknown Owner';
